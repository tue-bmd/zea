...
@backend_equality_check(
    backends=["tensorflow"],  # no torch: its `torch.compile` branch fails in the workers
    gt_backend=GT_BACKEND,
)  # no numpy, which has no autograd
...
def test_get_gradient_and_value_jit_fn_unsupported_for_torch():
    """Jitting gradient_and_value is not supported on the torch backend."""
    from zea.backend.autograd import AutoGrad

    with pytest.raises(NotImplementedError):
        AutoGrad().get_gradient_and_value_jit_fn()
