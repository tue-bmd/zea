...
def test_prepare_parameters_leaves_unset_parameters_to_the_caller():
    ...
    assert "sos_map" not in inputs and "map_grid_x" not in inputs
...
def _subprocess_simulate_two_dimensional_under_jit():  # pragma: no cover
    """`two_dimensional` is branched on with a Python `if` in `simulate_rf`, so it must be in
...
def test_simulate_two_dimensional_under_jax_jit():
    ...
    code = textwrap.dedent(inspect.getsource(_subprocess_simulate_two_dimensional_under_jit))
    code += "\n_subprocess_simulate_two_dimensional_under_jit()\n"
