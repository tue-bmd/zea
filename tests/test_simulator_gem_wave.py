"""``simulate_rf_gem_wave`` against ``simulate_rf``, feature by feature.
...
from zea.simulator import fft_length, simulate_rf, simulate_rf_gem_wave, smooth_size
...
CASES = {
    ...
    "elevation_lens_prunes": _case(_linear_probe(), elevation_lens=True, element_height=1e-3),
    "elevation_lens_all_inside": _case(_linear_probe(), elevation_lens=True, element_height=5e-3),
    "noise_and_tgc": _case(
...
def test_matches_simulate_rf(name):
    ...
    result = simulate_rf_gem_wave(**kwargs, band_db=None, n_fft=n_fft)
    _assert_close(reference, result)


def test_default_band_and_derived_fft_length_are_close():
    kwargs = _tensors(CASES["linear"])
    assert _correlation(simulate_rf(**kwargs), simulate_rf_gem_wave(**kwargs)) > 0.9999
...
def test_scatterers_beyond_record_are_dropped_without_wrapping():
    ...
    reference = simulate_rf(**kwargs)
    _assert_close(reference, simulate_rf_gem_wave(**kwargs, band_db=None, n_fft=1024))
    _assert_close(reference, simulate_rf_gem_wave(**kwargs, band_db=None))
...
def test_empty_phantom_gives_zeros():
    ...
    result = _np(simulate_rf_gem_wave(**_tensors(kwargs)))
...
def test_n_period_changes_the_pulse():
    kwargs = _tensors(CASES["linear"])
    short = simulate_rf_gem_wave(**kwargs, n_period=2.0)
    default = simulate_rf_gem_wave(**kwargs)
...
def test_under_jit_with_traced_geometry_needs_n_fft():
    ...
    jitted = jax.jit(simulate_rf_gem_wave, static_argnames=static)
...
def test_under_jit_closed_over_geometry_derives_n_fft():
    ...
    result = jax.jit(lambda p, m: simulate_rf_gem_wave(p, m, **kwargs))(positions, magnitudes)
...
def test_gradients_match_simulate_rf():
    ...
    result = jax.grad(loss(simulate_rf_gem_wave, band_db=None, n_fft=1024), argnums=(0, 1, 2, 3))(
        *args
    )
...
def test_simulate_op():
    ...
    gem = Simulate(with_batch_dim=False)
    reference = exact(**kwargs)[exact.output_key]
    result = gem(**kwargs, method="gem_wave", band_db=None, n_fft=512)[gem.output_key]
    _assert_close(reference, result)
...
def test_simulate_op_with_batch_dim():
    ...
    gem = Simulate(with_batch_dim=True)
    reference = exact(**kwargs)[exact.output_key]
    result = gem(**kwargs, method="gem_wave", band_db=None, n_fft=512)[gem.output_key]
    _assert_close(reference, result)
...
def test_single_transmit_goes_to_simulate_rf():
    ...
    np.testing.assert_array_equal(_np(simulate_rf(**kwargs)), _np(simulate_rf_gem_wave(**kwargs)))
...
def test_few_transmits_match_simulate_rf(n_tx):
    ...
    _assert_close(simulate_rf(**kwargs), simulate_rf_gem_wave(**kwargs))
