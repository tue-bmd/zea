...
class TestValidate:
    """Tests for File.validate() (lightweight structural check)."""

    def test_custom_map_passes_validate(self, tmp_path):
        """File.create() with a custom spatial map key should pass validate()."""
        import warnings

        n_frames, n_tx, n_el, n_ax = 2, 4, 8, 64
        raw = np.zeros((n_frames, n_tx, n_ax, n_el, 1), dtype=np.float32)
        custom_map = {
            "values": np.zeros((n_frames, n_ax, n_el), dtype=np.float32),
        ...
        with warnings.catch_warnings():
            ...
            File.create(
                path,
                data={"raw_data": raw, "my_custom_map": custom_map},
                scan=_scan_minimal(n_frames=n_frames, n_tx=n_tx, n_el=n_el),
            ).close()
    ...
    def test_unknown_flat_dataset_fails_validate(self, tmp_path):
        """A new-format file with an unrecognised flat dataset key should fail validate()."""
        import h5py

        path = tmp_path / "bad_flat.hdf5"
        with h5py.File(path, "w") as f:
            f.attrs["zea_version"] = "0.1.0"
            grp = f.create_group("data")
            grp.create_dataset("not_a_valid_key", data=np.zeros((2, 4, 64, 8, 1), dtype=np.float32))

        with File(path) as f:
            with pytest.raises(AssertionError, match="not_a_valid_key"):
                f.validate()

    def test_custom_map_warns_about_supported_types(self, tmp_path):
        """Adding a custom spatial map key should emit a UserWarning listing supported types."""
        ...
        assert any("my_overlay" in str(warning.message) for warning in w)
        assert any("envelope_data" in str(warning.message) for warning in w)

    def test_flat_array_custom_data_key_raises(self):
        """Passing a raw numpy array under a custom data key must raise TypeError."""
        from zea.data.spec import DataSpec
        ...
        with pytest.raises(TypeError, match="spatial map"):
            DataSpec(
                raw_data=np.zeros((n_frames, n_tx, n_ax, n_el, 1), dtype=np.float32),
                my_flat=np.zeros((n_frames, n_ax, n_el), dtype=np.float32),
            )

    def test_flat_array_custom_metadata_key_raises(self):
        """Passing a raw numpy array under a custom metadata key must raise TypeError."""
        from zea.data.spec import MetadataSpec

        with pytest.raises(TypeError, match="SignalND"):
            MetadataSpec(my_flat=np.zeros((100,), dtype=np.float32))
