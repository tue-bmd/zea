...
class TestDataValidationErrors:
    ...
    def test_image_values_frame_broadcast(self):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            # Framed (regression)
            img_framed = Image(values=np.zeros((2, 16, 12), dtype=np.uint8))
            assert img_framed.values.shape == (2, 16, 12)

            # Broadcast (no leading n_frames)
            img_broadcast = Image(values=np.zeros((16, 12), dtype=np.uint8))
            assert img_broadcast.values.shape == (16, 12)

            # Broadcast values combined with broadcast coordinates
            img_both_broadcast = Image(
                values=np.zeros((16, 12), dtype=np.uint8),
                coordinates=np.zeros((16, 12, 3), dtype=np.float32),
            )
            assert img_both_broadcast.values.shape == (16, 12)
            assert img_both_broadcast.coordinates.shape == (16, 12, 3)
            ...
            img_values_broadcast_coords_framed = Image(
                values=np.zeros((16, 12), dtype=np.uint8),
                coordinates=np.zeros((2, 16, 12, 3), dtype=np.float32),
            )
