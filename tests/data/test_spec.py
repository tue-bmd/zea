...
class TestMultiTrackProbeConsistency:
    ...
    def _make_track(self, n_el: int = 4, geom_offset: float = 0.0, element_width: float = None):
    ...
    def test_matching_probe_geometry_passes(self):
        ...
        track = self._make_track(geom_offset=0.0)
        FileSpec(tracks=[track, track])  # should not raise

    def test_mismatched_probe_geometry_raises(self):
        """Two tracks with different probe_geometry must raise ValueError."""
        track_a = self._make_track(geom_offset=0.0)
        track_b = self._make_track(geom_offset=1.0)
        with pytest.raises(ValueError, match="probe_geometry"):
    ...
    def test_mismatched_element_width_raises(self):
        ...
        track_a = self._make_track(element_width=0.0003)
        track_b = self._make_track(element_width=0.0006)
