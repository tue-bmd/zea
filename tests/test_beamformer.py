...
def test_tof_correction_reduce_das_matches_full(probe_geometry, flatgrid):
    """reduce_das=True must equal the delay-and-sum of the full aligned output.

    The fused low-memory path sums over elements and transmits inside the
    transmit scan; that must be numerically equivalent (up to float rounding)
    to materializing the full (n_tx, n_pix, n_el, n_ch) tensor and summing.
    """
    # n_ax must be large enough that the round-trip delays land in range;
    # otherwise every pixel clips and the TOF output is trivially zero.
    n_tx, n_ax, n_ch = 3, 1500, 2  # IQ data to also exercise phase rotation
    inputs = _make_tof_inputs(probe_geometry, flatgrid, n_tx=n_tx, n_ax=n_ax, n_ch=n_ch)
    inputs["f_number"] = 1.0  # exercise the f-number mask path as well

    full = keras.ops.convert_to_numpy(tof_correction(**inputs))
    reduced = keras.ops.convert_to_numpy(tof_correction(reduce_das=True, **inputs))

    n_pix = flatgrid.shape[0]
    assert reduced.shape == (n_pix, n_ch)
    assert np.abs(reduced).max() > 1e-3, "test is vacuous if the TOF output is all zeros"
    # DAS = sum over elements (axis 2) then over transmits (axis 0).
    expected = full.sum(axis=2).sum(axis=0)
    np.testing.assert_allclose(reduced, expected, rtol=1e-4, atol=1e-4)
    return reduced


def test_tof_correction_reduce_das_rejects_sos_map(probe_geometry, flatgrid):
    """The fused low-memory path is homogeneous-only and must reject a sos_map."""
    inputs = _make_multistatic_inputs(probe_geometry, flatgrid)
    with pytest.raises(ValueError, match="homogeneous"):
        tof_correction(reduce_das=True, **inputs)


@backend_equality_check(decimal=2)  # see note on the reduce_das test above
def test_tof_correction_reduce_das_pfield_matches_full(probe_geometry, flatgrid):
    """reduce_das with flat_pfield must equal pfield-weighted DAS of the full output.

    Folding the per-(pixel, transmit) pressure-field weights into the fused pass
    must match weighting the full aligned tensor then summing.
    """
    # n_ax large enough that delays are in range (see note above); a zero TOF
    # output would make the pfield-effect check below vacuous.
    n_tx, n_ax, n_ch = 3, 1500, 2
    inputs = _make_tof_inputs(probe_geometry, flatgrid, n_tx=n_tx, n_ax=n_ax, n_ch=n_ch)
    n_pix = flatgrid.shape[0]
    rng = np.random.default_rng(0)
    flat_pfield = (np.abs(rng.standard_normal((n_pix, n_tx))) + 0.1).astype(np.float32)

    full = keras.ops.convert_to_numpy(tof_correction(**inputs))  # (n_tx, n_pix, n_el, n_ch)
    reduced = keras.ops.convert_to_numpy(
        tof_correction(reduce_das=True, flat_pfield=flat_pfield, **inputs)
    )
    assert reduced.shape == (n_pix, n_ch)
    # weight per (tx, pix), then sum over elements and transmits.
    weighted = full * np.transpose(flat_pfield)[:, :, None, None]
    expected = weighted.sum(axis=2).sum(axis=0)
    np.testing.assert_allclose(reduced, expected, rtol=1e-4, atol=1e-4)
    # pfield must actually change the result vs the unweighted reduction.
...
def test_tof_correction_reduce_dmas_matches_full(probe_geometry, flatgrid):
    ...
    The fused low-memory DMAS reduces each transmit with the O(n_el) identity
    inside the scan; that must match materializing the full aligned tensor and
    running the naive O(n_el^2) DMAS (outer product, drop the diagonal, sum).
    """
    n_tx, n_ax, n_ch = 3, 1500, 2  # DMAS requires IQ
    inputs = _make_tof_inputs(probe_geometry, flatgrid, n_tx=n_tx, n_ax=n_ax, n_ch=n_ch)
    inputs["f_number"] = 1.0  # exercise the f-number mask path as well

    full = keras.ops.convert_to_numpy(tof_correction(**inputs))  # (n_tx, n_pix, n_el, 2)
    reduced = keras.ops.convert_to_numpy(
        tof_correction(reduce_das=True, reduce_beamformer="delay_multiply_and_sum", **inputs)
    )
    n_pix = flatgrid.shape[0]
    assert reduced.shape == (n_pix, n_ch)
    assert np.abs(reduced).max() > 1e-3, "test is vacuous if the TOF output is all zeros"

    # Naive DMAS reference: sum_tx sum_{i<j} c_i c_j / sqrt(|c_i c_j|).
    c = full[..., 0] + 1j * full[..., 1]  # (n_tx, n_pix, n_el)
    prod = c[..., :, None] * c[..., None, :]
    eps = keras.backend.epsilon()
    z = prod / np.sqrt(np.abs(prod) + eps**2)
    n_el = c.shape[-1]
    z = z * (1.0 - np.eye(n_el)) / 2.0  # drop the diagonal, halve (each pair counted twice)
    expected_c = z.sum(axis=(2, 3)).sum(axis=0)  # over element pairs, then transmits
    expected = np.stack([expected_c.real, expected_c.imag], axis=-1)
    np.testing.assert_allclose(reduced, expected, rtol=2e-3, atol=1e-3)
    return reduced


def test_tof_correction_reduce_dmas_requires_iq(probe_geometry, flatgrid):
    """The fused DMAS reduction needs IQ data (n_ch=2) and must reject RF (n_ch=1)."""
    inputs = _make_tof_inputs(probe_geometry, flatgrid, n_tx=2, n_ax=1500, n_ch=1)
    with pytest.raises(ValueError, match="IQ data"):
        tof_correction(reduce_das=True, reduce_beamformer="delay_multiply_and_sum", **inputs)
