...
def fetcher_for(file: h5py.File) -> Fetcher | None:
    ...
    if source is not None and str(source).startswith(HF_PREFIX):
        # Not the environment variables directly: `get_token` reads those *and* the token
        # file `hf auth login` writes, so a logged-in user reaches a gated repo without
        # having to also export HF_TOKEN.
        token = get_token()
