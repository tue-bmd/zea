...
def beamform_scanline(
    ...
    **tof_kwargs,
    ...
    """Scanline (line-by-line) beamforming: one transmit per image line.

    In conventional focused (scanline) imaging each transmit is focused along a
    single scan line and is used to reconstruct only that line.
    ``beamform_scanline`` implements this directly: transmit ``n`` is
    delay-and-summed along its own beam grid ``scanline_grids[n]`` and the
    per-transmit lines are stacked into an image, *without* compounding
    transmits onto a shared grid. The image is thus assembled from the transmit
    that actually insonified each line, as in classic B-mode scanline imaging.

    This is the natural reconstruction for acquisitions that transmit one
    focused beam per image line — walking-focus linear scans and steered
    focused sector scans. Compounding many focused transmits onto a common grid
    (e.g. retrospective transmit beamforming) is a different operation; see
    :class:`zea.ops.Beamform` with pressure-field weighting for that route.

    Works for both RF (``n_ch=1``) and IQ (``n_ch=2``) input.

    Args:
        data (Tensor): RF or IQ data of shape ``(n_tx, n_ax, n_el, n_ch)``
            (``n_ch=1`` for RF, ``n_ch=2`` for IQ), or batched data of shape
            ``(batch, n_tx, n_ax, n_el, n_ch)``.
        scanline_grids (Tensor): Per-transmit beam-line pixel positions
            ``(x, y, z)`` of shape ``(n_tx, n_line, 3)`` in meters. Line ``n`` is
            beamformed from transmit ``n`` only. See
            :func:`zea.beamform.pixelgrid.scanline_pixel_grid`.
        t0_delays (Tensor): Per-element transmit delays ``(n_tx, n_el)``.
        tx_apodizations (Tensor): Transmit apodizations ``(n_tx, n_el)``.
        sound_speed (float): Speed of sound in m/s.
        probe_geometry (Tensor): Element positions ``(n_el, 3)``.
        initial_times (Tensor): Per-transmit time offsets ``(n_tx,)``.
        sampling_frequency (float): Sampling frequency in Hz.
        demodulation_frequency (float): Demodulation (carrier) frequency in Hz
            (only used for IQ data).
        f_number (float): Receive f-number. ``0`` disables f-number masking.
        polar_angles (Tensor): Steering angles ``(n_tx,)`` in radians.
        focus_distances (Tensor): Focus distances ``(n_tx,)`` in meters.
        t_peak (Tensor): Waveform peak times ``(n_tx,)`` in seconds.
        transmit_origins (Tensor): Beam origins ``(n_tx, 3)``.
        **tof_kwargs: Forwarded to :func:`tof_correction` (e.g. lens correction).

    Returns:
        Tensor: Beamformed lines of shape ``(n_tx, n_line, n_ch)`` for unbatched
        input, or ``(batch, n_tx, n_line, n_ch)`` for batched input, summed over
        the receive aperture (line ``n`` from transmit ``n``). Envelope-detect
        the lines to obtain a B-mode image: for IQ input take the magnitude of
        the two channels, for RF input use the analytic (Hilbert) signal.
    """
    ...
    if data.ndim == 4:
        ...
        raise ValueError(
            "Expected data with shape (n_tx, n_ax, n_el, n_ch) or "
            f"(batch, n_tx, n_ax, n_el, n_ch), got {data.shape}."
        )

    def _beamform_single_frame(frame):
        def _beamform_single_transmit(
            data_tx,
            grid_tx,
            t0_tx,
            apod_tx,
            ...
            tof = tof_correction(
                ops.expand_dims(data_tx, axis=0),
                grid_tx,
                ops.expand_dims(t0_tx, axis=0),
                ops.expand_dims(apod_tx, axis=0),
                sound_speed,
                probe_geometry,
                ops.expand_dims(initial_time_tx, axis=0),
                sampling_frequency,
                demodulation_frequency,
                f_number,
                ops.expand_dims(angle_tx, axis=0),
                ops.expand_dims(focus_tx, axis=0),
                ops.expand_dims(t_peak_tx, axis=0),
                ops.expand_dims(origin_tx, axis=0),
                **tof_kwargs,
            )
            # tof: (1, n_line, n_el, n_ch) -> sum over receive elements
            return ops.sum(tof, axis=2)[0]

        return vmap(_beamform_single_transmit)(
            frame,
            scanline_grids,
            t0_delays,
            tx_apodizations,
    ...
    lines = vmap(_beamform_single_frame)(data)
