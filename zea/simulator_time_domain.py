...
def simulate_rf_td(
    ...
    A faster alternative to :func:`zea.simulator.simulate_rf` that produces equivalent RF
    data without a per-scatterer, per-frequency Fourier synthesis. Each scatterer
...
def get_pulse_waveform(center_frequency, sampling_frequency, n_period=4, n_samples=129):
    ...
    This is the time-domain counterpart of :func:`zea.simulator.get_pulse_spectrum_fn`: an
    even, zero-centered pulse whose spectrum matches the windowed sine used by
    :func:`zea.simulator.simulate_rf`. The pulse length ``n_samples`` is a fixed (static)
    sample count so the pulse has a compile-time-known shape; the Hann window zeros any
