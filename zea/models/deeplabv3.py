...
def DilatedSpatialPyramidPooling(dspp_input):
    ...
    """
    dims = ops.shape(dspp_input)
    x = layers.AveragePooling2D(pool_size=(dims[-3], dims[-2]))(dspp_input)
    x = convolution_block(x, kernel_size=1, use_bias=True)
    out_pool = layers.UpSampling2D(
        size=(dims[-3] // ops.shape(x)[1], dims[-2] // ops.shape(x)[2]),
...
def DeeplabV3Plus(image_shape, num_classes, pretrained_weights=None):
    ...
    input_a = layers.UpSampling2D(
        size=(image_shape[0] // 4 // ops.shape(x)[1], image_shape[1] // 4 // ops.shape(x)[2]),
        interpolation="bilinear",
    )(x)
    ...
    x = layers.UpSampling2D(
        size=(image_shape[0] // ops.shape(x)[1], image_shape[1] // ops.shape(x)[2]),
        interpolation="bilinear",
    )(x)
...
def ASPP(x, filters):
    ...
    pool = layers.UpSampling2D(size=(ops.shape(x)[1], ops.shape(x)[2]), interpolation="bilinear")(
        pool
    )
