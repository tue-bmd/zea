...
class DiffusionModel(DeepGenerativeModel):
    ...
    def _init_operator_and_guidance(self, operator, guidance):
        """
        Initializes the operator and guidance function for the diffusion model based on provided specifications.
        
        Accepts operator and guidance as string keys, configuration dictionaries, or instantiated objects. Instantiates and assigns the appropriate operator and guidance function, ensuring required parameters (such as `image_range`) are set when necessary. Raises a ValueError if invalid types are provided or if guidance is specified without an operator.
        """
    ...
    def call(self, inputs, training=False, network=None, **kwargs):
        """
        Runs the score network on the given inputs using the specified or default network.
        
        If `network` is not provided, uses the main network during training and the exponential moving average (EMA) network during inference.
        
        Parameters:
            inputs: Input tensor to the score network.
            training (bool): Whether to run in training mode.
            network: Optional network instance to use instead of the default.
        
        Returns:
            The output of the selected score network applied to the inputs.
        """
        if network is None:
    ...
    def sample(self, n_samples=1, n_steps=20, seed=None, **kwargs):
        """
        Generates samples from the diffusion model by running the reverse diffusion process starting from Gaussian noise.
        
        Parameters:
        	n_samples (int): Number of samples to generate.
        	n_steps (int): Number of diffusion steps to perform.
        	seed: Optional random seed generator for reproducibility.
        
        Returns:
        	Samples generated by the reverse diffusion process.
        """
    ...
    def linear_diffusion_schedule(self, diffusion_times):
        """
        Compute signal and noise rates for a linear diffusion schedule based on the provided diffusion times.
        
        Parameters:
            diffusion_times: A tensor representing the diffusion time steps.
        
        Returns:
            signal_rates: Tensor of signal rates for each diffusion step.
            noise_rates: Tensor of noise rates for each diffusion step.
        """

        def _compute_alpha_t(t):
            """
            Computes the cumulative product of (1 - diffusion rates) up to time step t for a linear diffusion schedule.
            
            Parameters:
                t: The current diffusion time step.
            
            Returns:
                The cumulative product of (1 - diffusion rates) up to step t.
            """
    ...
    def denoise(
        ...
        """
        Predicts the noise component from noisy images and computes the corresponding denoised images.
        
        Parameters:
            noisy_images: Noisy input images at a given diffusion step.
            noise_rates: Noise rates associated with the diffusion step.
            signal_rates: Signal rates associated with the diffusion step.
            training: Boolean indicating whether the model is in training mode.
            network: Optional; specifies which network variant to use for prediction.
        
        Returns:
            pred_noises: Predicted noise components.
            pred_images: Denoised images reconstructed from the noisy inputs and predicted noise.
        """
    ...
    def reverse_diffusion(
        ...
        """
        Runs the reverse diffusion process to generate images from initial noise, optionally starting from partially noised samples.
        
        Parameters:
            initial_noise: The initial noise tensor to start the reverse diffusion.
            diffusion_steps (int): Number of reverse diffusion steps to perform.
            initial_samples: Optional tensor of initial samples for warm-starting the diffusion process.
            initial_step (int): Step index to begin the reverse diffusion from.
            stochastic_sampling (bool): If True, uses stochastic (DDPM) sampling; otherwise, deterministic sampling is used.
            seed (keras.random.SeedGenerator | None): Optional random seed generator for reproducibility.
            verbose (bool): If True, displays a progress bar during sampling.
            track_progress_type (Literal[None, "x_0", "x_t"]): If set, tracks and stores intermediate results ("x_0" for predicted images, "x_t" for noisy images).
            disable_jit (bool): If True, disables JIT compilation for the diffusion loop.
            training (bool): If True, runs the network in training mode; otherwise, inference mode.
            network_type (Literal[None, "main", "ema"]): Specifies which network variant to use ("main" or "ema"). If None, selects based on the `training` flag.
        
        Returns:
            pred_images: The generated images after completing the reverse diffusion process.
        """
        ...
        def step_fn(step, loop_state):
            """
            Performs a single step of the reverse diffusion process, updating the noisy and predicted images.
            
            Parameters:
                step (int): The current diffusion step.
                loop_state (tuple): A tuple containing the current noisy images, predicted images, and random seed.
            
            Returns:
                tuple: Updated noisy images, predicted images, and seed for the next diffusion step.
            """
