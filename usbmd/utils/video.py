...
class ScanConverter:
    ...
    def __init__(
        ...
        self.max_buffer = deque(
            maxlen=max_buffer_size
        )  # Buffer for max values (for normalization)
    ...
    def persistence_MA(self):
        ...
        if self.n_persistence > 1:
            ...
            img = np.mean(
                np.array([self.img_buffer[i] for i in range(max_index)]), axis=0
            )
        else:
...
class ScanConverterTF(ScanConverter):
    ...
    def convert(self, img):
        ...
        img = self.resize(img)
        img = tf.cast(
            (img + self.dynamic_range) * (255.0 / self.dynamic_range), tf.uint8
        )
    ...
    def compression(img):
        """Logarithmic compression"""
        return 20 * tf.math.log(img) / tf.math.log(10.0)
