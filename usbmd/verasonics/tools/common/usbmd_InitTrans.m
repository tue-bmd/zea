...
function Trans = usbmd_InitTrans(transducerName, frequency, numElements)
    ...
    if strcmp(transducerName,'CL15-7') || ...
        ...
        error(['Transducer ', transducerName, ' is not supported.']);
    end
...
function Trans = computeCustomTrans(Trans, transducerName)
    ...
    elseif strcmp( transducerName, 'L15-7io')
            ...
            Trans.impedance      = 50;
            Trans.ConnectorES    = [2,1,5,6,10,9,13,14,18,17,21,22,25,26,29,30,33,34,37,38,41,42,45,46,49,50,...
                53,54,57,58,61,62,65,66,69,70,73,74,77,78,81,82,85,86,89,90,93,94,97,98,101,102,105,106,109,...
                110,113,114,117,118,121,122,125,126,128,127,124,123,120,119,116,115,112,111,108,107,104,103,...
                100,99,96,95,92,91,88,87,84,83,80,79,76,75,72,71,68,67,64,63,60,59,56,55,52,51,48,47,44,43,40,...
                39,36,35,32,31,28,27,24,23,20,19,16,15,12,11,8,7,3,4]';
            Trans.ElementSens    = gausswin(101,pi)';
...
function verifyValidFrequency(frequency)
    if ~any(usbmd_g_SupportedTxFreqMHz == frequency)
