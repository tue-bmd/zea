...
def segment(tensor, number_erasing = 0, min_clip = 0):
    ...
    rows_left = np.array([102, 103, 103, 104, 104, 105, 105, 106, 106, 107, 107, 107, 108, 108, 109, 109, 109, 110, 110, 111, 111])
    
    # Bottom right curve (manual fit)
    cols_right = np.linspace(89, 111, 23).astype(np.int32)
    rows_right = np.array([111, 111, 111, 110, 110, 110, 109, 109, 109, 108, 108, 107, 107, 107, 106, 106, 105, 105, 104, 104, 103, 103, 102])   
...
def accept_shape(tensor):
    ...
    for i in range(len(cols)):
        ...
        # tensor[rows_top[i], cols[i]] = 1.5
        # tensor[rows_bot[i], cols[i]] = 1.5
...
def cartesian_to_polar_matrix(cartesian_matrix, tip=(61, 7), r_max = 107, angle=0.79, interpolation='nearest'):
    ...
    polar_values = griddata(data_points, data_values, new_points, method=interpolation, fill_value=0)
...
if __name__ == '__main__':
            ...
            with h5py.File(os.path.join(self.path_in, h5file), 'r') as file:
                ...
                if accepted:
                    ...
                    if val_counter < self.num_val:
                        out_dir = os.path.join(self.path_out, 'val', h5file.replace('.hdf5', ''))
                        out_h5 = os.path.join(self.path_out_h5, 'val', h5file)
                    elif test_counter < self.num_test:
                        out_dir = os.path.join(self.path_out, 'test', h5file.replace('.hdf5', ''))
                        out_h5 = os.path.join(self.path_out_h5, 'test', h5file)
                    else:
                        out_dir = os.path.join(self.path_out, 'train', h5file.replace('.hdf5', ''))
                        out_h5 = os.path.join(self.path_out_h5, 'train', h5file)
                else:
                    out_dir = os.path.join(self.path_out, 'rejected', h5file.replace('.hdf5', ''))
                ...
                for i, im in enumerate(tensor):
                    ...
                    if accepted:
                        polar_im = cartesian_to_polar_matrix(im, interpolation='cubic')        # [nearest, linear, cubic]
                        np.save(os.path.join(out_dir, f'polar{str(i).zfill(3)}.npy'), polar_im)
                        polar_im_set = np.concatenate([polar_im_set, np.expand_dims(polar_im, axis=0)], axis=0)
    ...
    h5_files = [file for file in h5_files if file.endswith('.hdf5') and file not in files_done]
    print(f'Files left to process: {len(h5_files)}')

    processor = H5Processor(path_in, path_out, path_out_h5)
    with ProcessPoolExecutor() as executor:
        results = list(tqdm(executor.map(processor.process_h5_file, h5_files), total=len(h5_files)))
